...
area_ghm <- fread("~/Desktop/covid-results/area_ghm_effects_2023-05-02.csv") %>%
  select(species, Estimate, LCL, HCL, sig_code) %>%
  mutate("ghm_sig" = ifelse(sig_code != "ns_add", 1, 0)) %>%
  distinct(species, ghm_sig)

area_sg <- fread("~/Desktop/covid-results/area_sg_effects_2023-05-02.csv") %>%
  select(species, Estimate, LCL, HCL, sig_code) %>%
...
niche_ghm <- fread("~/Desktop/covid-results/niche_ghm_effects_2023-05-11.csv") %>%
  select(species, Estimate, LCL, HCL, sig_code) %>%
  mutate("ghm_sig" = ifelse(sig_code != "ns_add", 1, 0)) %>%
  distinct(species, ghm_sig)

niche_sg <- fread("~/Desktop/covid-results/niche_sg_effects_2023-05-11.csv") %>%
  select(species, Estimate, LCL, HCL, sig_code) %>%
  mutate("sg_sig" = ifelse(sig_code != "ns_add", 1, 0)) %>%
  distinct(species, sg_sig)
